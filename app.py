...
from api.routers.recruitment import router as recruitment_router, initialize_recruitment
...
ROUTERS = (system_router, auth_router, admin_router, recruitment_router, research_graph_router, feedback_router, chat_router,
...
def _parse_cors_origins(raw: Optional[str]) -> List[str]:
    ...
    return origins or ["http://127.0.0.1:3000", "http://localhost:3000", "http://127.0.0.1:3001", "http://localhost:3001"]

_CORS_ALLOW_ORIGINS = _parse_cors_origins(os.getenv("CORS_ALLOW_ORIGINS", ""))
_CORS_ALLOW_ORIGIN_REGEX = os.getenv("CORS_ALLOW_ORIGIN_REGEX", r"https://.*\.ngrok-free\.app|https://.*\.ngrok\.app").strip() or None
...
def _assert_embedding_dim_matches_pinecone() -> None:
    ...
    if not embedding_backend_is_real():
        print(f"[startup] WARNING: embedding backend={get_embedding_backend()} "
...
def create_app() -> FastAPI:
    application = FastAPI(title="CausalGraph API", version="1.1.0")
    application.add_middleware(CORSMiddleware, allow_origins=_CORS_ALLOW_ORIGINS,
        allow_origin_regex=_CORS_ALLOW_ORIGIN_REGEX, allow_credentials=True,
        allow_methods=["*"], allow_headers=["*"])
    application.add_middleware(GZipMiddleware, minimum_size=1024)
    @application.middleware('http')
    async def private_contract_responses(request, call_next):
        response = await call_next(request)
        if request.url.path.startswith('/legal/'):
