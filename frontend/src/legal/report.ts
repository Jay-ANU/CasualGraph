...
const EVIDENCE: Record<string, string> = { supported: '已有依据', quick: '极速审查，未经复核', unconfirmed: '待核实', rejected: '复核已排除' };
...
export function ReviewReport(review: Review): string {
  const out = ['# 合同审查报告', '', `审查编号：${review.id}`, `审查状态：${OUTCOME[reviewOutcome(review) || ''] || STATUS[review.status] || review.status}`, `模型：${review.profile?.model?.id || '历史记录未提供'}`, `我方角色：${review.profile?.our_role || '未记录'}`, `审查方式：${TIER_LABEL[tierOf(review)]}`, '', review.notice];
  ...
  if (review.skills?.length) out.push('', '## 审查技能', ...review.skills.map(skill => `${escaped(skill.name)}（v${escaped(skill.version)}）：${escaped(skill.reason)}`), '审查技能是审查方法与检查清单，不是法律依据。');
  if (review.research?.status === 'skipped') out.push('', '## 法律检索', '本档位不检索法规；法律意见引用的法条为模型引用，待核对。');
  else if (review.evidence_health) out.push('', '## 法律检索', `取得来源：${review.evidence_health.topics_with_sources}/${review.evidence_health.queried_topics} 项；版本待核验：${review.evidence_health.version_pending} 个来源。`, review.evidence_health.notice);
  out.push('', '## 审查意见');
  for (const [index, f] of review.findings.entries()) {
    out.push('', `### ${index + 1}. ${escaped(f.title)}`, `类别：${KIND_LABEL[f.kind] || f.kind}；依据状态：${EVIDENCE[findingStatus(f)]}；风险等级：${findingStatus(f) === 'supported' ? SEVERITY[f.severity] || f.severity : findingStatus(f) === 'quick' ? `${SEVERITY[f.severity] || f.severity}（未经复核）` : '不作为已成立风险计级'}；原文段落：${f.block_id || '待确定插入位置'}`, '', '原文：', escaped(f.original_quote), '', '影响：', escaped(f.impact), '', '分析：', escaped(f.reason));
