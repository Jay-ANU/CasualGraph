...
export function PolicyEditor({ catalog, policies, busy, onSave, onArchive }: {
  ...
  return <main id="legal-main" tabIndex={-1} className="lv-policy-page">
    <div className="lv-policy-inner">
      <h1>公司规范</h1>
      <p className="lv-lede">写下你们在合同上的硬性要求，例如预付款上限、必须的审批。审查时会逐条对照，并在报告里保存当时的版本。规范是内部要求，不会被当作法律规定。</p>
      <div className="lv-policy-layout">
        <section className="lv-policy-list" aria-labelledby="legal-policy-list-title">
          <div className="lv-section-head"><h2 id="legal-policy-list-title">正在使用</h2><span>{policies.length} 条</span></div>
          {!policies.length && <div className="lv-policy-empty"><BookOpen {...ic} size={20} /><p>还没有公司规范</p><small>可以先审查合同，再逐步补充付款、责任和审批要求。</small></div>}
          {policies.map(p => <article key={p.id}>
            <div className="lv-policy-title"><h3>{p.title}</h3><small>第 {p.version} 版</small></div>
            <span className="lv-policy-type">{p.contract_type} · {p.our_roles?.length ? p.our_roles.join(' / ') : '不限我方角色'}</span>
            <p>{p.text}</p>
        ...
        <form className="lv-policy-form" onSubmit={e => { e.preventDefault(); if (validScope && !busy) onSave(draft, editing); }}>
          ...
          <label className="lv-field"><span className="lv-field-label">规范标题</span>
            <input className="lv-input" required maxLength={150} value={draft.title} onChange={e => setDraft({ ...draft, title: e.target.value })} placeholder="例如：采购预付款上限" /></label>
          ...
          {scene && <fieldset className="lv-field lv-fieldset" disabled={busy}>
            <legend className="lv-field-label">只在我方是以下角色时适用<span className="lv-optional">不选表示全部角色</span></legend>
          ...
          {!validScope && <p className="lv-note is-warn" role="alert">适用合同或角色与当前目录不匹配，请重新选择后再保存。</p>}
          <label className="lv-field"><span className="lv-field-label">审查要求</span>
            <textarea className="lv-textarea" required minLength={5} maxLength={2500} rows={6} value={draft.text} onChange={e => setDraft({ ...draft, text: e.target.value })}
              placeholder="例如：我方为采购方时，预付款超过合同总价的 30% 需业务负责人批准。这是公司要求，不是法定比例。" /></label>
          <p className="lv-hint">只有组织管理员可以修改规范。请不要填写与审查无关的机密信息。</p>
          <div className="lv-inline-actions">
