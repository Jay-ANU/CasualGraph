...
export type Profile = { scenario?: ContractScenario; our_party?: { block_id: string; quote: string }; transaction_context?: TransactionContext; review_mode?: 'standard' | 'multi_agent'; transaction_date?: string; model?: { id: string; provider: string }; our_role?: string; contract_type?: string; instructions?: string };
...
export type Review = { transaction_brief?: TransactionBrief | null; evidence_health?: EvidenceHealth; retrieval?: { rule_id: string; status: string; provider: string; warnings: string[] }[]; draft_check?: DraftCheck; draft_approval?: { fingerprint: string; final_hash: string; user_id: string }; collaboration?: Collaboration; metrics?: { model_calls?: number }; id: string; contract_id?: string; status: string; stage: string; resumable: boolean; error?: string; findings: Finding[]; coverage: Coverage[]; sources: Source[]; decisions: Record<string, Decision>; policies: Policy[]; notice: string; profile?: Profile; engine_version?: number; intake?: { facts: Fact[] }; summary?: { high: number; medium: number; low: number; checked_rules: number; total_rules: number; needs_confirmation: number }; progress?: { phase: string; completed: number; total: number }; batch_errors?: Record<string, string> };
export type Capabilities = { scenario_catalog_version?: number; scenario_catalog?: ScenarioCatalog; audit_foundation_version?: number; draft_release_version?: number; upload_disclosure?: { version: string; notice: string; storage_region: string }; collaboration_version?: number; model_configured: boolean; encryption_configured: boolean; review_engine_version?: number; followup_questions?: boolean; law_search: { provider: string; notice: string } };
export type Model = { id: string; family: string };
export type Catalog = { models: Model[]; families: string[]; default_model: string; notice: string };
export type Answer = { id: string; question: string; answer: string; status?: string; uncertain?: boolean; block_refs?: { block_id: string; quote: string }[]; citations?: Citation[]; notice?: string; model?: string };
