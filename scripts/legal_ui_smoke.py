...
try:
    with sync_playwright() as p, capture_failure():
        ...
        page.get_by_role('heading',name='签字之前，先把合同看明白').wait_for()
        ...
        expect(page.get_by_role('radiogroup',name='我方角色').get_by_role('radio',checked=True)).to_have_count(0)
        ...
        for scene in SCENARIOS['scenarios']:
            ...
            expect(page.get_by_role('radiogroup',name='我方角色').get_by_role('radio',checked=True)).to_have_count(0)
            expect(page.get_by_role('radiogroup',name='我方角色').get_by_role('radio')).to_have_count(len(scene['roles']))
            page.get_by_role('radiogroup',name='我方角色').get_by_role('radio',name=scene['roles'][0]['value'],exact=True).check()
        page.get_by_label('合同类型',exact=True).select_option('销售合同')
        expect(page.get_by_role('radiogroup',name='我方角色').get_by_role('radio',checked=True)).to_have_count(0)
        page.get_by_role('radiogroup',name='我方角色').get_by_role('radio',name='销售方',exact=True).check()
        ...
        page.get_by_text('补充交易背景（可选）',exact=True).click()
        ...
        expect(page.get_by_text('请输入不带单位的数字，最多两位小数，例如 1000000.05。',exact=True)).to_be_visible()
        page.get_by_label('交易金额',exact=True).fill('1000000.05')
        consent=page.get_by_label('同意将脱敏后的合同正文、补充说明和适用的公司规范，经 YData 模型网关发送给所选 AI 模型分析。')
        ...
        page.get_by_label('合同类型',exact=True).select_option('销售合同');page.get_by_role('radiogroup',name='我方角色').get_by_role('radio',name='销售方',exact=True).check()
        ...
        page.get_by_role('heading',name='这份合同有 1 处需要关注').wait_for()
        ...
        page.get_by_label('同意将已选修改（脱敏版本）发送给原审查模型进行核验。').check()
        page.get_by_role('button',name='核验所选修订',exact=True).click()
        page.get_by_label('我已核对全部已选修改及剩余风险，确认生成修订稿。').check()
