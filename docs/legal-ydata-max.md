...
YData credentials are now routed server-side by model family because one YData API key can be scoped to one model company. Configure any combination of `YDATA_GPT_API_KEY`, `YDATA_CLAUDE_API_KEY`, `YDATA_DEEPSEEK_API_KEY`, `YDATA_KIMI_API_KEY` and `YDATA_GLM_API_KEY` as Fly backend secrets. The existing `YDATA_API_KEY` remains supported as a legacy credential so the currently working provider does not break during migration. A dedicated family key always wins for that family; the backend never silently falls back from a configured dedicated key to another company's key.

The destination remains fixed to `https://www.ydata.space/v1`. For automatic discovery the backend calls `GET /models` once for each distinct configured credential, filters a dedicated credential to its declared family, merges and de-duplicates the resulting text-chat model IDs, and returns one catalog to the frontend. The frontend never receives or chooses credentials. If one dedicated provider key is unauthorized, rate-limited or returns no matching chat model while another key is healthy, the healthy models remain available and the affected family is reported in `unavailable_families`. If no configured key produces a usable model, the catalog fails closed.

At generation time the selected model ID is validated against that merged catalog, its GPT/Claude/DeepSeek/Kimi/GLM family is resolved, and `POST /chat/completions` uses only that family's dedicated key or the legacy key when no dedicated key exists. Keys cannot be shared across two dedicated family slots; such a configuration is rejected. There is no cross-provider credential fallback and no automatic model substitution.

By default, text-chat candidates in the GPT (including o-series), Claude, DeepSeek, Kimi/Moonshot and GLM families are discovered automatically. Embedding, image, speech, realtime and other non-text models are excluded. A listed ID is not a guarantee that the gateway supports that model on the chat-completions route; unavailable/unsupported models fail explicitly.

`LEGAL_YDATA_DEFAULT_MODEL` optionally selects an existing merged catalog ID. `glm-5.2` is preferred only when actually listed. `LEGAL_YDATA_MODELS` optionally supplies an administrator-reviewed JSON array of exact model IDs when a gateway credential does not expose a models route or the operator wants a restricted list. Every configured model is still routed by family to its corresponding key. Do not populate the list with guessed models. The short-lived in-memory catalog cache is invalidated when any YData credential, configured inventory or default model changes.

Generation uses the supplied chat protocol with per-family output token budgeting, no universal temperature/thinking/JSON-mode flags, local strict JSON validation and no automatic generation retry. Truncation, authorization failure, rate limits, network errors and invalid JSON cannot become published review conclusions. Functional tests use synthetic model IDs/responses; production inference still requires real YData credentials for the desired families.
