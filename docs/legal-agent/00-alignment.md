...
状态：**方向已确认（2026-09-24），进入 Phase 0 执行**。确认内容见第 7 节；工作包任务书在 `docs/legal-agent/wp/`。
...
> 面向**企业法务**的 **合同逐条审阅 agent**：上传合同 → **自动脱敏** → 按条款切分 → 法务**打开一条一条看、逐条审核**（AI 给出每条的解读、风险、与公司立场的偏差和建议改法，法务逐条确认或批注）→ 汇总产出审阅意见表与批注版合同。

工作单元是**条款**，不是聊天消息。对话问答只是侧栏辅助。

### 2.2 V1 覆盖的用户旅程

1. 创建 matter（案件/项目），邀请成员。
2. 上传合同（PDF / DOCX / 文本；扫描件 OCR 放 V2）。
3. **脱敏预览与确认**：系统在本地识别当事人名称、自然人姓名、证件号、统一社会信用代码、电话、邮箱、地址、银行账号等，替换为一致的占位符（如 `【甲方】`、`[PERSON_1]`、`[ID_1]`）；原文只在本地加密保存；用户可增删条目后确认。**未确认脱敏的文件不会被送往任何第三方模型或向量 API。**
4. 系统按条款切分成条款树（第 X 条 / 1.1 / (a) / Article 1 / Section 1.1），显示审阅进度 N/M。
5. **逐条审**：每条展示原文 + AI 分析（含义、风险等级、与 playbook / 公司立场的偏差、建议改法、关联条款与定义的引用）。法务操作：通过 / 标记问题 / 修改建议 / 加批注 / 跳过。状态：未审 / 已审 / 有问题。
6. 全文级检查：缺失条款、前后矛盾、关键要素表（当事人、期限、价款与支付、违约与赔偿、责任上限、知识产权、保密、解除、管辖与争议解决…）。
7. 随时提问（侧栏对话），答案引用可点击跳到对应条款并高亮。
8. 导出：审阅意见表（XLSX/CSV）、审阅备忘录（DOCX）、批注 / 建议改法版合同（DOCX）。导出前**反脱敏**还原真实名称。
9. 全程可追溯：脱敏记录（只记类别与数量，不记原值）、每一步工具调用、每条引用、每次导出都进 matter 的审计日志。
...
### G. 脱敏（de-identification）是送模前的强制关卡

```
上传原件 ──► 解析 ──► 本地脱敏引擎 ──► 用户预览/确认 ──► "可送模"文本
   │                      │                                    │
   └─ 加密保存原件          └─ 映射表（加密、按文档）              ├─► 切分 / 向量化 / 模型调用（只见占位符）
                                                               └─► 展示层 / 导出层 反向映射还原
```

- 识别**全部在本地完成**，不能靠云端模型：结构化字段用规则（身份证、统一社会信用代码、手机、邮箱、银行账号、护照、车牌）；当事人名称用合同结构启发式（"甲方：XXX有限公司"、签署栏、定义条款）+ 词典；自然人姓名与地址用本地 NER（候选：spaCy 中英小模型，或轻量规则 + 用户确认）。
- 一致替换：同一实体全文同一占位符，模型才能跨条款推理"甲方 / 乙方"的权利义务。
- 映射表按文档加密存储在 matter 下；提问文本先做正向映射（用户输入的真名 → 占位符），模型输出做反向映射后再展示；导出物还原真名。
- 向量化用的也是脱敏后文本（DeepInfra / Pinecone 同样是第三方）。
- 策略按组织可配置：默认脱敏类别见 D9；金额与日期默认保留（审阅需要），可开启。
- 审计只记录类别与数量，不记录原值；日志与错误信息里禁止出现原文。
- 前端提供脱敏预览 / 手工增删 / 确认的步骤；未确认前文档状态为"待脱敏"，所有模型入口拒绝处理。

### B′. 条款树是一等公民

- 解析后先做**条款切分**（`legal/clause_segmenter.py`）：从 DOCX 编号与样式、PDF 页文本的编号模式（第 X 条、一、二、1.1、(一)、(a)、Article、Section、Clause）构建层级条款树；识别不确定时由本地规则回退，必要时用模型辅助分段（只处理脱敏后文本）。
- 检索与引用的 chunk **等于条款或子条款**，而非固定 1500 字符窗口；引用落到 `文件 · 第 N 条 · 第 P 页`。
- 逐条审阅的状态、批注、AI 结论都挂在条款节点上。
...
| 1.5 条款切分 | `legal/clause_segmenter.py`：中英文编号模式、层级树、DOCX 编号优先、PDF 文本回退；chunk = 条款 | 5 份样本合同（中英各有）切分准确率抽检；每个条款节点有编号、标题、原文范围、页码 |
| 1.6 脱敏管道 | `legal/redaction/`：本地检测器（规则 + 当事人启发式 + 本地 NER）、一致占位符、加密映射表、正向 / 反向映射、策略配置、预览 API（列出 / 增删 / 确认）、"未确认不可送模"的状态门 | 单测：每类样例的召回；映射往返无损；未确认文档调用任何模型入口返回 409；日志中不出现原值的测试 |
...
| 2.5 逐条审阅引擎 | `legal/review.py`：对单条款调用 agent（读本条 + 关联定义 / 交叉引用 + playbook 立场）产出结构化结论 `{summary, risk_level, issues[], suggested_text, citations[]}`；全文级检查（缺失条款、矛盾、关键要素表）；结论挂到条款节点；用户操作（通过 / 标记 / 改建议 / 批注）持久化 | 结构化输出 schema 校验；每条结论的引用能定位回条款；缺失条款检查对 NDA 样本命中预期清单 |
...
| 4.2 脱敏预览 | 上传后进入"脱敏确认"步骤：按类别列出识别结果、原文高亮、增删、确认；状态门提示 | Playwright：上传 → 看到识别列表 → 确认 → 文档变为可审 |
| 4.3 逐条审阅工作台 | 左：条款树 + 进度；中：当前条款原文 + AI 分析 + 操作（通过 / 标记 / 改建议 / 批注）+ 上一条 / 下一条；右：侧栏对话（引用跳条款）与阅读器（原件页高亮，本地反脱敏显示） | Playwright：打开合同 → 逐条前进 → 标记一条 → 进度更新 → 侧栏提问引用跳转 |
| 4.4 全文检查与导出 UI | 缺失条款 / 矛盾 / 关键要素表视图；选 playbook；导出三种文件 | Playwright：跑完一次全文检查并下载文件 |
...
Phase 0 已开工，任务书见 `docs/legal-agent/wp/phase0-backend.md`、`docs/legal-agent/wp/phase0-frontend.md`。每个 Phase 结束汇报一次。

## 7. 已确认的方向与采用的默认值（2026-09-24）

| # | 决定 | 来源 |
|---|---|---|
| D1 | 目标用户：**企业法务**。司法辖区 V1 法域中立；界面中英双语、默认中文 | 用户确认（用户）+ 默认（法域 / 语言） |
| D2 | V1 场景：**合同逐条审阅**。核心工作流是"打开一条一条看、逐个审核"，见 2.1 / 2.2 | 用户确认 |
| D3 | 模型：provider 层同时支持 Anthropic 与 OpenAI 兼容；**默认沿用 DeepSeek**（现有密钥与部署不变），可按组织切换；原生 tool use 为硬要求；任何文本送模前必须先过脱敏关卡 | 默认 |
| D4 | 保留 FastAPI + React + Vercel/Fly；后端拆模块、前端迁 Vite；不做全新重写 | 默认 |
| D5 | 离题功能**直接删除**（招聘、桌面宠物及其端点、ESG demo、因果推断页、图谱可视化、ESG 抽取与指标、legacy 目录、过期 CI）。历史在 git 里，随时可恢复 | 默认 |
| D6 | 图谱：ESG 因果图谱的全部界面与端点移除；`graph/neo4j_store.py` 与 `rag/graph_context.py` 暂保留不注册，Phase 2 不纳入工具集；V2 再评估法务实体图 | 默认 |
| D7 | 合规：**上传后必须脱敏**，脱敏前不送第三方模型 / 向量 API；私有模型选项预留接口 | 用户确认（脱敏）+ 默认（私有模型预留） |
| D8 | 1.11 的安全问题随 Phase 0 一起修（大部分随离题端点删除而消失，其余单独修） | 默认 |
| D9 | **待确认**：脱敏默认类别 = 当事人 / 组织名称、自然人姓名、证件号、统一社会信用代码、电话、邮箱、地址、银行账号；金额与日期默认**不**脱敏（审阅需要）。是否需要把金额也脱敏？是否允许用户对某份文件"跳过脱敏"？ | 待用户答复，不阻塞 Phase 0 |
